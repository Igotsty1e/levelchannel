...
- `DEV_EXTRA_ALLOWED_ORIGINS=` (dev/test only; comma-separated extra origins trusted by `enforceTrustedBrowserOrigin`. Ignored when `NODE_ENV=production` — an accidental prod env entry cannot widen the trusted origin set. Useful for LAN device testing and tunnel UIs (cloudflared / ngrok). See `lib/security/request.ts::getAllowedOrigins`.)
...
- `SBP_ENABLED=false` (PAY-SBP-REMOVAL 2026-05-20; operator gate for `POST /api/payments/sbp/create-qr`. Default off — `/pay` UI button was removed because the CloudPayments merchant terminal doesn't have SBP activated. Set to literal `'true'` to revive the route once CP-side activation lands. Exact-match guard rejects truthy strings like `'1'`/`'yes'`.)
