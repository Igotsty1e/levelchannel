...
- TLS-required Postgres connections to remote hosts: `lib/db/pool.ts` auto-detects literal loopback hosts (`localhost` / `127.0.0.1` / `::1` / `[::1]`) as no-TLS (loopback — single-server deploys are valid) and forces `ssl: { rejectUnauthorized: true }` on every non-loopback host. Loopback classification lives in `lib/security/local-host.ts::isLiteralLoopbackHostname` (single source of truth shared with `lib/api/cron-auth.ts`). `*.local` is NOT loopback (mDNS attack surface — Codex 2026-05-07). `*.localhost` and `0.0.0.0` are only loopback for URL-validation surfaces (`lib/api/origin.ts` + `lib/payments/config.ts`) via `isLoopbackOriginUrl`. Production refuses `DB_SSL=disable` and `DB_SSL_REJECT_UNAUTHORIZED=false` ONLY for non-loopback hosts — disabling TLS on a remote Postgres in prod is the actual leak; on the loopback path TLS is meaningless. The JS-side `ssl` option overrides any `?sslmode=...` URL hint, so the policy is owned in code, not in the URL
...
## Cron-route trust boundary (BCS-OP-ROLLOUT)

The 6 cron-triggered routes under `/api/cron/calendar/*`
(pull / push / intents / renew-channels / revive-blocked / reconcile)
are reachable from systemd timers on the same VPS, NEVER from the
public internet. Two-layer gate in `lib/api/cron-auth.ts`:

1. **Loopback Host check** — the request's `Host` header must be
   `127.0.0.1`, `localhost`, or `::1` (optionally a value from
   `CRON_TRUSTED_HOST`). External callers via nginx see `Host:
   levelchannel.ru` upstream and receive **404** (not 401) — by
   design, so the routes' existence is hidden from external scans.

2. **Bearer secret** — `Authorization: Bearer ${CRON_SHARED_SECRET}`
   required after the host gate passes. Constant-time compare.
   503 if the env is unset; the cron runner stops cleanly rather
   than silent-allowing.

`CRON_SHARED_SECRET` is auto-generated by `scripts/activate-prod-
ops.sh` on first run (`openssl rand -hex 32`) and appended to
`$ENV_FILE`. Operator never sets it manually. Rotation: rewrite the
env-file line, restart `levelchannel.service`, and update the
cron-runner env (same file in the single-VPS layout) — new secret
