  ...
  return (
    <>
      ...
      >
        ...
        Google-календарём). Конфликт-пробник дополнительно умеет рассылать
        персональные письма учителям по их слотам — это включается флагом
        <code>CONFLICT_UNRESOLVED_TEACHER_FANOUT_ENABLED=1</code> в env
        пробника и Next-сервера (по умолчанию ВЫКЛ; в карточке «Последнее
        уведомление» отражается только operator-ветка). Плюс отдельная
        служба напоминаний для учащихся — она шлёт каждому одно письмо
        за окно (по умолчанию 60&nbsp;минут) до начала занятия. Чтобы
        реальные тики и письма пошли, нужно запустить{' '}
        <code>scripts/activate-prod-ops.sh</code> на VPS — пробники, чьи
        systemd-таймеры ещё не установлены, покажут «Данные недоступны»
        ниже. Тестовое письмо можно отправить прямо отсюда (это уже
        работает до активации таймера). Пороги редактируются прямо здесь:
