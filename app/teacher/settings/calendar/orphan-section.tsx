...
export function OrphanSection({ initialSlots }: Props) {
  ...
  async function ignoreAll() {
    if (busy) return
    if (!confirm(`Очистить устаревшие связи на ${initialSlots.length} слотах?`)) {
      return
    }
    setBusy('all')
    setError(null)
  ...
  return (
    ...
    >
      ...
      >
        Эти слоты были связаны с событиями в Google в прошлой
        интеграции (до того, как вы переподключили календарь). События
        в Google остались — это нормально, они ваши. Нажав
        «Очистить связь», вы только разорвёте локальную ссылку: слот
