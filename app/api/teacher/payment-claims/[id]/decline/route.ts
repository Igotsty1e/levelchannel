...
import { declineClaim } from '@/lib/payments/sbp-claims'
import {
  enforceRateLimit,
...
export async function POST(
  ...
  const parsed = await readJsonObjectOr400(request).catch(() => null)
  const note =
    parsed && parsed.ok && typeof parsed.body.note === 'string'
      ? parsed.body.note
      : null

  const r = await declineClaim(guard.account.id, id, note)
  if (!r.ok) {
    const status =
