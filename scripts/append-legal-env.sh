#!/bin/bash
#
# Append missing NEXT_PUBLIC_LEGAL_* env vars to /etc/levelchannel.env.
#
...
# edit /etc/levelchannel.env by hand, then re-run /usr/local/bin/
# levelchannel-autodeploy.
#
# After a successful run, trigger one autodeploy cycle:
#   /usr/local/bin/levelchannel-autodeploy
...
if [ "$changed" = "1" ]; then
  ...
  echo "  /usr/local/bin/levelchannel-autodeploy"
  echo ""
