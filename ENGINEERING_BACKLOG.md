...
- ~~**Refund-flow Phase 7.**~~ **Closed across Waves 50/51/52 (2026-05-11, PRs #161, #162, #163).** Three-stage rollout:
  ...
  - Stage B (Wave 51, PR #162) — `POST /api/admin/refunds` admin endpoint. Books a reversal in tx + emits new `payment.refund.recorded` audit event (migration 0037 widens the CHECK). Contract: 201 / 400 (invalid_body / unsupported_kind / partial_refund_not_supported / refund_exceeds_allocation / invalid_refunded_at) / 404 (allocation_not_found) / 409 (already_refunded surfaces existing reversalId). Scope: `kind='lesson_slot'` only; `kind='package'` returns 400 unsupported_kind. 6 integration tests. Codex round caught a HIGH (route accepted `refundedKopecks < amount` which would mark slot fully unpaid because the read paths drop on row existence, not amount match) — fixed before merge with strict equality + `partial_refund_not_supported` error code.
  ...
  - **Out-of-scope for this wave:** `kind='package'` refunds, partial reversals (would need SUM-over-reversals refactor of the read paths), payment-gateway-side automation (today: operator pushes the actual money via CloudPayments dashboard and records the reversal here after).
