...
### #2 MEDIUM-LOW — CSP unsafe-inline (Wave 11: PRs 1 + 1.1 + 4 shipped; PR 3 upstream-blocked)
**Status:** PRs 1, 1.1, 4 shipped 2026-05-08/09 (#92, #94, this-PR). `style-src` now strict (`'self'` only); `style-src-attr 'unsafe-inline'` covers ~700+ inline JSX `style={...}` attrs. Dead `fonts.googleapis.com`/`fonts.gstatic.com` allowlist entries dropped (next/font self-hosts since Next 13+). PR 3 (drop `'unsafe-inline'` from `script-src`) BLOCKED: Next.js 16.2.4 doesn't auto-stamp `nonce` on framework-emitted RSC hydration payload `<script>` blocks — confirmed independently of file convention (rename to `proxy.ts` per Next.js 16 deprecation didn't help) and independently of Turbopack vs webpack hypothesis. Issue [#88](https://github.com/Igotsty1e/levelchannel/issues/88) tracks. Resume PR 3 when upstream Next.js fixes the auto-stamp gap; the policy template + middleware + nonce flow are already in place, so unblocking is a one-line edit in `lib/security/csp.ts`.
