...
- **BUG-2026-05-13-2 — нет удаления тарифа, только деактивация.** Доработка: добавить hard-delete для тарифа. UI — красная корзинка рядом с тарифом, подтверждение модалкой («Вы уверены, что хотите удалить тариф?»). Проверить, что удалять можно только тарифы без активных привязок (allocations / package_purchases) — иначе только деактивация, с подсказкой почему. Не делать каскад без явного решения.
- **BUG-2026-05-13-3 — у тарифа нет обязательного поля «длительность занятия».** Сейчас тариф можно завести без `duration_minutes`, и потом 90-минутный тариф можно применить к 60-минутному слоту (или наоборот). Доработка: сделать `duration_minutes` обязательным полем при создании тарифа; добавить выбор (30/45/60/90 или произвольное), DB-level NOT NULL после миграции существующих строк. Заодно — гейт в book/allocate, чтобы тариф не клеился к слоту другой длительности.
- **BUG-2026-05-13-4 — непонятное поле `Sort` (=0) в тарифах.** Сейчас в админке у тарифа есть поле `Sort` со значением `0`. Доработка: переименовать в человекочитаемое («Порядок отображения», «Позиция в списке учеников» — что бы оно ни значило), добавить подсказку под полем. Проверить, где это поле реально используется в рендере — если нигде, убрать.
- **BUG-2026-05-13-5 — непонятная шапка страницы тарифов.** Доработка копирайта: переписать заголовок и подзаголовок страницы тарифов так, чтобы админ с первого взгляда понимал, что это и зачем.
- **BUG-2026-05-13-6 — непонятный текст на вкладке «Возвраты».** Доработка копирайта: переписать пояснительный текст вкладки «Возвраты» бизнесово, без технических терминов. Должно быть понятно, что это за возвраты (refunds vs reversals?), как они возникают, и что админ может с ними сделать.
- **BUG-2026-05-13-7 — непонятное поле «Порядок» (=100) на вкладке «Пакеты».** Аналогично BUG-…-4, но для пакетов. Доработка: переименовать или дать понятную подсказку; проверить, влияет ли реально на сортировку в каталоге.
