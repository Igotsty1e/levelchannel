...
- ~~подключить error tracking~~ — **закрыто 2026-04-29**: Sentry @sentry/nextjs v10 + `instrumentation.ts` (Node/Edge) + `instrumentation-client.ts` (browser) + `app/global-error.tsx`. Project `mastery-zs/levelchannel`. Smoke event прошёл end-to-end. Активация в production = добавить `SENTRY_DSN` + `NEXT_PUBLIC_SENTRY_DSN` в `__LEVELCHANNEL_ENV_FILE__`. Подробно — `OPERATIONS.md §9` Sentry
