...
- добавить failure alerting **на webhook-контур** (CloudPayments check/pay/fail) — uptime probe не покрывает; нужен либо отдельный signal от backend на стабильный поток успешных webhook'ов, либо pull-side monitor через query `payment_audit_events` (теперь, когда audit log на проде, можно делать GH Actions cron, который query'ит `select count(*) from payment_audit_events where event_type = 'webhook.pay.processed' and created_at > now() - interval '1 hour'` и алертит если 0 при наличии созданных заказов)
