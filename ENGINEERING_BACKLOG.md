...
- **BCS-DEF-1-TEST-FILLOUT** — Test coverage fillout for the BCS-DEF-1 conflict-unresolved probe (logged 2026-05-19, expanded 2026-05-19 fresh-paranoia round). Foundation test (`tests/integration/admin/conflict-unresolved-foundation.test.ts`, 6 cases) covers only CHECK extension + resolver default/env paths. Outstanding: (1) execFile-driven `tests/integration/scripts/conflict-unresolved-alert.test.ts` covering all `verdict_kind` paths (no_offenders / alert_sent / dedup_skip / alert_send_failed / config_missing / error); (2) fairness regression — 100×3-teacher seed asserting per-teacher cap arithmetic + cross-teacher visibility; (3) `tests/admin/probe-status.test.ts` structural test of `PROBE_NAMES` length + `isProbeName('conflict-unresolved')`; (4) `conflict-unresolved` execFile branch inside `probe-resolver-integration.test.ts`; (5) regression pin for the `pg_get_constraintdef` preflight branch in `app/api/admin/settings/alerts/[probe]/test-send/route.ts:119-149` (wave-paranoia BLOCKER#1 closure — currently only auth-flow / calendar-pathology paths in `tests/integration/admin/alerts-obs.test.ts`); (6) unit-mocked DB-reading helpers (`readOffenderCounts`, `readOffenderRows`, `readPerTeacherOmittedCounts`, `readFingerprintTuples`) via mocked `pg.Pool.query`; (7) per-key POST/DELETE tests in `operator-settings-route.test.ts` for the 4 new `CONFLICT_UNRESOLVED_*` keys. Not a prod blocker — fairness is enforced by SQL planner via `ROW_NUMBER() OVER (PARTITION BY teacher_account_id ...)` — these are regression pins, not correctness gates. ~150-220 LOC across 6 files.
- **BCS-DEF-1-COPY-STYLE-SWEEP** — Copy-style sweep across BCS-DEF-1 surfaces to align with `docs/content-style.md` (logged 2026-05-19 fresh-paranoia round 7). PR #365 closed obvious "Тестовый алерт" → "Тестовое уведомление" (per content-style.md:117). Outstanding: (1) consider adding slot-id column to `/admin/slots` table so the conflict-unresolved email's slot-ID grep instruction is actually actionable (currently only teacher email + start_at are visible columns; see `app/admin/(gated)/slots/slots-manager.tsx:721,731`); (2) audit all probe email bodies for "слот" usage in learner-facing contexts (admin-side is OK per content-style.md:53); (3) verify "Уведомления оператора" page-title spelling vs. "Алерты" in any remaining nav-link copy. Small follow-up; mainly auditing, not refactor.
