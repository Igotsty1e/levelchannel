...
### #2 MEDIUM-LOW — CSP `script-src 'unsafe-inline'` (Wave 11 in flight; PR 1 shipped, PR 2 BLOCKED upstream)
**Status:** PR 1 shipped (#92, 2026-05-08). PR 2 blocked: Next.js 16.2.4 + Turbopack does NOT auto-stamp `nonce` on framework-emitted RSC hydration payload `<script>` blocks. Verified live on prod after PR 1 deploy: middleware correctly sets `x-nonce` + CSP per request (3 unique nonces in 3 calls), but rendered HTML shows 5 inline scripts without `nonce=`. PR 3 (drop `'unsafe-inline'`) cannot ship without breaking hydration. Plan updated `docs/plans/csp-hardening.md` §PR 2 with 5 forward-paths; recommended next try is webpack-mode build (option 4). Tracking issue: [#88](https://github.com/Igotsty1e/levelchannel/issues/88) (will be updated with the auto-stamp finding).
