...
- **BCS-DEF-1-TEST-FILLOUT** — Test coverage fillout for the BCS-DEF-1 conflict-unresolved probe (logged 2026-05-19 from re-paranoia §0d round-2 BLOCKER#2). Foundation test (`tests/integration/admin/conflict-unresolved-foundation.test.ts`, 6 cases) covers only CHECK extension + resolver default/env paths. Outstanding: (1) execFile-driven `tests/integration/scripts/conflict-unresolved-alert.test.ts` covering all `verdict_kind` paths (no_offenders / alert_sent / dedup_skip / alert_send_failed / config_missing); (2) fairness regression — 100×3-teacher seed asserting per-teacher cap arithmetic + cross-teacher visibility; (3) `tests/admin/probe-status.test.ts` structural test of `PROBE_NAMES` length + `isProbeName('conflict-unresolved')`; (4) `conflict-unresolved` execFile branch inside `probe-resolver-integration.test.ts`. Not a prod blocker — fairness is enforced by SQL planner via `ROW_NUMBER() OVER (PARTITION BY teacher_account_id ...)` — these are regression pins, not correctness gates. ~120-180 LOC across 4 files.
