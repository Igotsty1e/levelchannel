...
Cross-project rule (`~/.claude/CLAUDE.md` § legal-rf): any work
involving Russian legal source-of-truth (public oferta, privacy
policy, consent text, on-server consent metadata) must flow through
`legal-rf-router → profile skill → legal-rf-qa` before landing.

This guardrail makes that rule **mechanically enforced** in the
LevelChannel repo. It does not verify that the pipeline literally ran
(we cannot inspect a different agent's session). It does enforce that
every commit that mutates a regulated file leaves a written record of
intent, making silent bypass visible at the commit / PR level.
...
list in [`scripts/legal-pipeline-check.sh`](../scripts/legal-pipeline-check.sh)
in the same commit and run the pipeline.
...
Required when any file in the protected scope is touched (added,
modified, or deleted). Free-form value, but two recommended shapes.
...
Use only when the diff is provably non-substantive (typo, formatting,
dead link, asset path). The reason is part of the audit trail; an
over-broad `trivial-fix` claim will surface in future review.
...
| Shared logic | `scripts/legal-pipeline-check.sh` | Single matching / trailer implementation reused by the hook and the CI. |
...
trailer is the cheaper path. Project policy ([`AGENTS.md`](../AGENTS.md)
§ Risk discipline) treats `--no-verify` as a hard stop that requires
...
1. Copy `scripts/legal-pipeline-check.sh`, edit `LEGAL_PATHS` and
...
4. Add a `postinstall` script that sets `core.hooksPath`.
