...
**Status:** **Wave A SHIPPED 2026-05-08.** PR1 #106 (backend foundation), PR2 #107 (component skeleton), PR3 #108 (operator surface), PR4 #109 (teacher surface). Read-only contract closed end-to-end on prod (`/api/slots/calendar`, `/admin/slots` calendar tab, `/teacher`). PR3b (drag-paint + drag-move on operator calendar) intentionally deferred — see Wave A close-out comment in #109. Wave B (learner-side picker) and Wave C (teacher self-create) remain for future sessions; this plan stays in `docs/plans/` as the audit trail for what shipped.

**Original plan (v4 post-Codex round 3, 18-22h estimate) follows:**
