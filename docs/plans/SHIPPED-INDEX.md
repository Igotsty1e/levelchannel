...
## 2026-06-27 metrika-rename-landing epic (4 PRs)

- **`metrika-rename-landing-2026-06-27.md`** — началось как «почини CSP-ошибку + переименуй Приём оплат→Учёт оплат + вычитай лендинг»; Codex-paranoia вскрыл, что «CSP-ошибка» = заблокированная Яндекс.Метрика с Webvisor, не раскрытая в Политике (152-ФЗ). Owner: включить Метрику корректно. **#749** 152-ФЗ disclosure (Метрика+Webvisor в privacy/consent, `PERSONAL_DATA_DOCUMENT_VERSION` 2026-06-27.1, миграция 0142 privacy/personal_data v2, P0 убрал divergent CSP из `.htaccess`) → **#750** честный копирайт про трекеры + rename Приём→Учёт (cabinet UI) + вычитка landing-v3 (тире, 5 вкладок, до 3 учеников, Слоты→Время занятий) → **#751** включение (explicit CSP nonce + Метрика public-pages-only через usePathname) → **#752** epic-end wave follow-up (wss://mc.yandex.ru в connect-src, nonce fail-closed, accurate copy, semantic revert). Status: SHIPPED 2026-06-27. Codex-Paranoia: plan SIGN-OFF round 4/4 (owner-authorized), wave SIGN-OFF round 2/2. Canary clean (SHA 97864d9). Legal через legal-rf-router→commercial→qa (баннер не обязателен; Webvisor accepted-risk + masking). **Follow-up:** устаревшие SEO learn-страницы (learn/free «1 ученик», crm-for-tutors тарифы Базовый/Расширенный) — см. ENGINEERING_BACKLOG.
