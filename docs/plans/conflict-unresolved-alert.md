...
связаться с учителем напрямую (email слева). Для отмены или
переноса конкретного слота — найдите слот по teacher email + start_at
на странице /admin/slots (slot ID в email body — для psql lookup;
⚠️ страница /admin/slots сейчас не рендерит slot ID-колонку, см.
`app/admin/(gated)/slots/slots-manager.tsx`; добавление колонки
запланировано в BCS-DEF-1-COPY-STYLE-SWEEP). Если событие в Google
уже удалено, конфликт очистится автоматически на следующем
pull-worker тике (~30 минут).
...
- ~~`buildEmail(offenders, total, thresholdsSnapshot)`~~ — ⚠️ **As shipped:** signature is `buildEmail(offenders, counts, perTeacherOmitted)` per `scripts/conflict-unresolved-alert.mjs:308`. The "и ещё N" line is driven by `perTeacherOmitted.get(teacherAccountId) > 0` (per-teacher, not global) per `:327` / `:345`, NOT by a `total > offenders.length` check. Original draft signature is historical.
...
> **⚠️ NOT AUTHORED — see §3 honesty block + BCS-DEF-1-TEST-FILLOUT.** The cases below describe the **target** integration test that would have shipped in the original draft scope. As-shipped: only `tests/integration/admin/conflict-unresolved-foundation.test.ts` exists (CHECK + resolver/default/env). The full execFile-driven test below was deferred to backlog. Do **not** assume any of these cases is regression-pinned.
...
> **⚠️ As shipped:** the original dedicated `probe-runs-conflict-unresolved-allowed.test.ts` was **not authored** as a separate file — the same two assertions live in `tests/integration/admin/conflict-unresolved-foundation.test.ts:23-48` (CHECK accept + CHECK reject), per §3 honesty block. The `alerts-obs.test.ts:183-217` CHECK extension WAS shipped (the integration bootstrap matches production).
...
Also **update** `tests/integration/admin/alerts-obs.test.ts:183-217` — the manual `CREATE TABLE probe_runs ... CHECK (...)` block now includes `'conflict-unresolved'` so integration tests bootstrap a CHECK matching production. *(This part IS shipped.)*
...
> **⚠️ NOT AUTHORED — see §3 honesty block + BCS-DEF-1-TEST-FILLOUT item 3.** The file does not exist in the tree. The runtime behaviour IS exercised indirectly via TypeScript's `Record<ProbeName, ...>` exhaustiveness check at the page render path, but the structural unit test described below was deferred.
