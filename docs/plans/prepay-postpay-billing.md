...
- **Refund flow inheritance** — refund wave shipped 2026-05-11..12 (Waves 50-64). The consumption ledger is the natural place to restore-package-on-refund via `restored_at`, and `restoreAllConsumptionsForPurchase` (Wave 53, `lib/billing/consumption.ts`) drops every active consumption when a `kind='package'` allocation is reversed. The `paid-state` SUM expression does a LATERAL `SUM(refunded_kopecks)` against `payment_allocation_reversals` and treats an allocation as fully paying while `SUM < amount_kopecks` (Wave 54 partial-reversal contract). The four read paths (`slotIsPaidByAllocations`, `listSlotPaidStatus`, `listSlotPaymentState`, `listAccountPostpaidDebt`) all agree on this binary all-or-nothing semantic. Wave 60 added gateway-side automation behind `BILLING_REFUND_GATEWAY_ENABLED` (durable `payment_refund_attempts` two-phase flow with idempotency-key support, `lib/billing/refund-attempts.ts`); Wave 61 added the reconcile worker (`scripts/refund-reconcile.mjs` + every-5-min systemd timer + `lib/billing/refund-reconcile.ts`) for the partial-success recovery path; Wave 62 bounded all CloudPayments fetch calls with `AbortController`; Wave 64 surfaced the reversal history under `/admin/refunds`.
