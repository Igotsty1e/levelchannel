...
The deletion-blocking guard is implemented at TWO points: (1) `lib/auth/accounts.ts` `requestAccountDeletion()` and (2) the cron-side anonymizer in `scripts/db-retention-cleanup.mjs` (or the dedicated `account-deletion-execute.mjs`, whichever owns the final-step anonymize). Both call the same `accountHasInFlightPackageGrant(accountId)` helper which evaluates BOTH branches A and B. PR 1 adds the predicate, both call sites, and integration tests covering: (a) schedule blocked when Branch A matches (pending order < 15 min); (b) schedule allowed when pending order is older than 15 minutes (Branch A stale); (c) schedule allowed with no pending order and no paid-not-granted; (d) execute blocked when pending order arrives between schedule and cron-fire (Branch A re-check at execute); (e) execute allowed when pending order resolved (paid AND granted) by cron-fire time; (f) **schedule blocked when Branch B matches** (paid order without `package_purchases` row); (g) execute blocked when Branch B matches at execute time (webhook still mid-retry). Branch B has no time bound — paid-not-granted is an indefinite block until operator reconciles via `/admin/payments`.
