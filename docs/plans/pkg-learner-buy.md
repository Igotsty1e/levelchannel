...
- **RISK-4 (3DS-callback redirect lacks token — NEW, scoped OUT of this wave):** `app/api/payments/3ds-callback/route.ts:171-173` redirects to `/thank-you?invoiceId=...` without `&token=` on the server side. The plain token is only known at order-init time (the hash is what's stored). To thread a token through 3DS round-trip, we'd need to either (a) pass plain token via CloudPayments `JsonData` so it round-trips, OR (b) mint a fresh single-use redirect token in receipt-token-gate. Both are non-trivial security-surface changes. **Impact for this wave:** users whose card requires 3DS during package purchase will see /thank-you's "Не удалось получить статус оплаты" message; the order still grants via the webhook, the package still lands, and the operator can confirm via /admin/payments/[invoiceId]. Documented limitation; tracked separately as `RECEIPT-3DS-TOKEN` follow-up wave. Mock + no-3DS production paths are fully covered.
