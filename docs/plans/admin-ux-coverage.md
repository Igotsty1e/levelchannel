...
| Priority | Item | Rationale |
|---|---|---|
| **P0** | **Wave PKG-RECON** — `/admin/accounts/[id]` reconciliation UI for `paid_not_granted` orders. Operator can "re-run grant" (calls `processPackageGrant` retry), "attach to correct account" (account-id mismatch repair), "refund" (delegate to existing refund machinery), "mark resolved" (audit-tagged dismiss). | Codex #3+#4: existing operational hole; raw-SQL-only workaround today |
| **P1** | **Wave PKG-LEARNER-BUY** — `/cabinet/packages` page + buy CTA. Server-side amount/duration/count/title/expiry from catalog (no operator-supplied amount). | User-observed product-owner gap |
