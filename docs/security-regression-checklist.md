...
- [ ] If your diff touched a public route, hit it once with curl
      to confirm shape.
- [ ] **Hit at least one route that exercises the shared
      `getDbPool()`** (e.g. `/cabinet`, `/admin/slots`,
      `/api/auth/me` with a valid cookie) — `/api/health` uses its
      own ad-hoc `Pool` and does NOT exercise the production-path
      pool factory. The 2026-05-07 hotfix landed because Wave 1.1's
      pool-init throw fired only on the shared pool path, while the
      health probe came back green from its private pool. Smoking
      ONLY the health endpoint is a known blind spot — close it on
      every deploy by hitting one route through the shared path.
