...
- **Body (2 параграфа):**
  ...
  Параграф 1: `Мы не передаём твои данные третьим лицам. Не показываем их в рекламе. Не передаём в аналитические системы. На сайте у нас один счётчик, который считает только клики по кнопкам — без имён, без email, без сумм.`
