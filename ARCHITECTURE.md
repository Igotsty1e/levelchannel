...
| `app/api/payments/sbp/create-qr` | POST | session + receipt-token | SBP-PAY (2026-05-19) — create CloudPayments-hosted SBP QR for the order; writer-side resolves account id via `lib/payments/order-account-resolver.ts` (admin/teacher rejected). Sets `payment_orders.payment_method='sbp'` at create-qr time |
