...
Read-only `/admin/settings/alerts` surface (ALERTS-OBS, 2026-05-17; extended by BCS-DEF-1 to 4 probes 2026-05-19). Four systemd cron alert probes emit JSON to journald + persist a per-tick row to `probe_runs` so the operator can see "last run", "last alert", effective thresholds (snapshot from row, NOT process env), and trigger a dry-run test email.
