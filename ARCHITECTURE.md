...
- [`lib/payments/cloudpayments.ts`](lib/payments/cloudpayments.ts) - building the server-side order and the widget intent for CloudPayments / CloudKassir. PKG-LEARNER-BUY LBL.0 (2026-05-16) made `buildCloudPaymentsWidgetIntent` accept an optional `{ receiptToken }` so the `successRedirectUrl` carries `&token=` and `/thank-you`'s status fetch is authorised on the widget-completed redirect (epic-end paranoia BLOCKER #2 closure). Server-side fallbacks that 303 the user to `/thank-you` outside the widget — `app/api/payments/3ds-callback/route.ts:172` and the same widget intent's `successRedirectUrl` when the receipt token isn't threaded — currently omit the plain token and rely on the parallel webhook-grant to land the package (the order itself is never lost; only the immediate `/thank-you` status fetch returns 401 until the operator views the order via `/admin/payments/[invoiceId]`). Tracked as known limitation **RECEIPT-3DS-TOKEN** — separate follow-up wave
...
- [`lib/auth/guards.ts`](lib/auth/guards.ts) - `requireAuthenticated`, `requireAuthenticatedAndVerified`, `requireAdminRole`, plus the Wave 2.1 / 1.3 learner-archetype gates `requireLearnerArchetype` and `requireLearnerArchetypeAndVerified` (deny-list on `admin` + `teacher`; legacy "no role" passes through as the implicit student archetype). Used by cabinet + admin + `/api/slots/*` route handlers
...
- [`app/api/checkout/package/[slug]/route.ts`](app/api/checkout/package/%5Bslug%5D/route.ts) - package-buy init. PKG-LEARNER-BUY LBL.0 (2026-05-16) refactor in place: guard via `requireLearnerArchetypeAndVerified` + post-guard `isLearnerArchetypeCandidate` (deletion-grace coverage; matches the cabinet page SoT), wrapped in `withIdempotency(scope = checkout:package:${slug}:${accountId})` to defeat replay across packages and accounts (Codex post-review HIGH closure). The body of the idempotency callback acquires a per-(accountId, durationMinutes) `pg_advisory_xact_lock(hashtextextended('pkg-buy:' || account || ':' || duration, 0))` on one `PoolClient`, evaluates both gates and runs the `INSERT INTO payment_orders` inside the same TX — Gate 1 (`accountHasPendingPackageGrantForDuration` → 409 `pending_package_in_flight`) and Gate 2 (`learnerHasActivePackageOfDuration` → 409 `already_owns_active_package`) cannot race against each other or against parallel POSTs from the same learner. After the TX commits, the route calls `buildCloudPaymentsWidgetIntent(order, { receiptToken })` for `provider=cloudpayments` and returns the intent in the response body as `checkoutIntent` (mock provider returns `checkoutIntent: null`). Different-duration purchases from the same learner proceed concurrently. Lock namespace `pkg-buy:` verified non-colliding with `pkg-recon:` / `pkg_consume:` / `cp:` / `legal:` (2026-05-16)
