...
- [`lib/scheduling/slots.ts`](lib/scheduling/slots.ts) - Phase 4 store ops + bulk-preview generator. Atomic UPDATE-with-`status='open'` re-assert for concurrent-book races; UTC-stored `start_at`, IANA tz at render. `/api/slots/*` (cabinet — list available + mine + book + cancel) and `/api/admin/slots/*` (admin — single + bulk-preview + bulk-create + edit + delete + cancel + book-as-operator). Cabinet UI at [`app/cabinet/lessons-section.tsx`](app/cabinet/lessons-section.tsx); admin UI at [`app/admin/slots/`](app/admin/slots) with weekday-grid bulk preview-deselect-commit flow
