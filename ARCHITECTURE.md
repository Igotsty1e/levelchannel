...
- [`lib/notifications/catalog.ts`](lib/notifications/catalog.ts) — shared notification event/channel copy catalog for teacher notification-preferences surfaces; pure module imported by both the client matrix and the server preference/read side to avoid text drift between bundles.
...
- [`lib/db/pool.ts`](lib/db/pool.ts) - the single pg Pool for the project (5 domain pools were consolidated 2026-04-29). `getDbPool()` throws when `DATABASE_URL` is missing; `getDbPoolOrNull()` does a silent skip. Cap via `DATABASE_POOL_MAX` env (default 10). Wave 1.1 added `resolveSslConfig(url, env)` which auto-detects literal loopback hosts (`localhost` / `127.0.0.1` / `::1` / `[::1]`) as no-TLS (loopback — single-server deploys with Postgres on the same VPS are a valid topology) and forces `ssl: { rejectUnauthorized: true }` on every non-loopback host. Loopback classification lives in [`lib/security/local-host.ts`](lib/security/local-host.ts) (`isLiteralLoopbackHostname`) — single source of truth shared with [`lib/api/cron-auth.ts`](lib/api/cron-auth.ts). `*.local` is NOT loopback (mDNS attack surface — Codex 2026-05-07). `*.localhost` and `0.0.0.0` are only loopback for URL-validation surfaces ([`lib/api/origin.ts`](lib/api/origin.ts) + [`lib/payments/config.ts`](lib/payments/config.ts)) via `isLoopbackOriginUrl`. Production refuses `DB_SSL=disable` and `DB_SSL_REJECT_UNAUTHORIZED=false` ONLY for non-loopback hosts. The JS-side `ssl` option overrides any `?sslmode=...` URL hint, so the policy is owned in code.
...
| `0106_calendar_drop_moscow_only_triggers.sql` | calendar-onboarding-cleanup (2026-06-05, Option A) | drops Moscow-only triggers from mig 0043 (`teacher_calendar_integrations_msk_only_guard` + `account_profiles_timezone_msk_guard_trg`) and their functions | any teacher with a profile.timezone in the existing 19-entry allowlist (mig 0069 unchanged) can now connect Google Calendar — Moscow no longer special-cased at the DB layer. App-layer gates in `app/api/teacher/calendar/google/{start,callback}` + `app/api/account/profile` + `/teacher/settings/calendar` SSR enforce timezone-required invariant. DB-level defense-in-depth (require-timezone trigger + no-clear-while-active trigger) DEFERRED to a follow-up PR to avoid rolling-deploy race window. Plan: `docs/plans/calendar-onboarding-cleanup-2026-06-05.md`. |
| `0107_calendar_require_timezone_triggers.sql` | calendar-onboarding-followup (2026-06-06) | adds the DB-level timezone-invariant triggers deferred from mig 0106 — `teacher_calendar_integrations_require_timezone_trg` (fires on every active|degraded write) + `account_profiles_timezone_required_when_integration_active_trg` (fires on INSERT/UPDATE/DELETE) | closes the steady-state TOCTOU race that app-layer gates alone can't reach. Both triggers take per-account `pg_advisory_xact_lock(hashtextextended('tz_invariant:' || account_id, 0))` BEFORE the cross-table SELECT so concurrent writers for the same account serialize (otherwise READ COMMITTED admits the race). Preflight LOCK + abort on grandfathered (active|degraded with NULL/missing profile) rows. Plan: `docs/plans/calendar-onboarding-followup-2026-06-06.md`. |
| `0108_push_channel_check_and_audit_events.sql` | BCS-DEF-4-PUSH (2026-06-06) | widens `learner_reminder_dispatches.channel` CHECK to add `'push'`; extends `skipped_reason` CHECK with `'no_push_subscription'` + `'push_helper_not_shipped'`; widens `auth_audit_events.event_type` CHECK with 5 `push.subscription.*` events | enables the Web Push branch of the learner reminder scheduler. CHECK widening only, no data rewrites. |
| `0109_learner_push_subscriptions.sql` | BCS-DEF-4-PUSH (2026-06-06) | NEW `learner_push_subscriptions` table — per-(account, browser-device) Web Push subscriptions; active-endpoint partial UNIQUE index (`endpoint` WHERE `unsubscribed_at IS NULL`); active per-account partial index; created_at desc index | per-device opt-in storage for Web Push lesson-start reminders. Cross-account endpoint reassignment serialized at app layer via advisory lock on `'push_sub:' || endpoint`; cap of 10 active subs per account with FIFO eviction; auto-unsubscribed by scheduler on Web Push 410/404. |
...
- [`migrations/0043_teacher_calendar_integrations.sql`](migrations/0043_teacher_calendar_integrations.sql) — per-teacher Google OAuth state, encrypted tokens (CALENDAR_ENCRYPTION_KEY: access_token_enc / refresh_token_enc; channel_token_enc added in migration 0054 AUDIT-SEC-4), sync_state + epoch + last_reconnected_at + last_pulled_at, channel triple. Originally shipped with MSK-only trigger + symmetric `account_profiles.timezone` guard; both DROPPED by [`migrations/0106_calendar_drop_moscow_only_triggers.sql`](migrations/0106_calendar_drop_moscow_only_triggers.sql) (2026-06-05). Timezone-required invariant moved to app layer per `docs/plans/calendar-onboarding-cleanup-2026-06-05.md`.
