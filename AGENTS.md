...
- **Auth and account layer:** `lib/auth/`, `lib/email/`, `app/api/auth/`
  ...
  - `app/api/auth/*` уже реализуют register / verify / login / logout / reset-request / reset-confirm / me. Полного cabinet UI пока нет, но auth backend уже production-bound по своим env и security gates.
- **Email transport:** `lib/email/`
  - `client.ts` — Resend SDK; falls back to `console.log` when `RESEND_API_KEY` is empty.
...
- Resend env: `RESEND_API_KEY` + `EMAIL_FROM`. Empty key is a console fallback только в dev; в production boot уже падает, если auth email-контур не сконфигурирован.
