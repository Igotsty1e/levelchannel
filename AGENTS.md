...
|---|---|---|
| **Plan/doc agreement BEFORE implementing a wave** | **`/codex-paranoia plan <file>`** | **MANDATORY** per `~/.claude/CLAUDE.md §Two-checkpoint paranoia pipeline`. Codex adversarially reviews the plan with hard cap 3 rounds; all BLOCKERs close before code starts. Wave 1.1's overzealous "refuse localhost in prod" failure mode is exactly the shape this checkpoint catches. |
| **Post-wave code review BEFORE `gh pr create`** | **`/codex-paranoia wave <commit-range>`** | **MANDATORY**. After the wave's complete functional unit is implemented (full module / pipeline — NOT half), the full diff goes through Codex's 3-round paranoia. Carries `Codex-Paranoia: SIGN-OFF round N/3` (or `ESCALATED` / `SKIPPED`) trailer in the commit body. Auditable via `git log --pretty=full | grep Codex-Paranoia:`. |
| **Before any wave that spans more than two PRs** | `/plan-eng-review` (then `/codex-paranoia plan`) | Independent eng-manager pass on the plan **before** code lands. `/plan-eng-review` is the human-mentor pass; `/codex-paranoia` is the Codex pass — both happen on substantive waves. 10 minutes saves a hotfix. |
