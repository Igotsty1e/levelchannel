...
// Batch variant for cabinet "К оплате" rendering — one query for
// many slots. Keys returned by slot id.
export async function slotsArePaidByAllocations(
  slotIds: string[],
): Promise<Map<string, SlotPaidStatus>> {
  const out = new Map<string, SlotPaidStatus>()
