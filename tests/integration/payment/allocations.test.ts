...
async function seedLearnerWithBookedSlot(args: {
  ...
  // SAAS-PIVOT Epic 6 Day 6 — /api/payments rejects non-plan-4
  // teachers with 422. Seed a plan-4 subscription + teacher role
  // grant so the slot-bound payment flow proceeds.
  ...
  await getDbPool().query(
    `insert into teacher_subscriptions (account_id, plan_slug, state)
       ...
       on conflict (account_id) do update
         set plan_slug = 'operator-managed', state = 'active'`,
    [teacher.id],
  )
