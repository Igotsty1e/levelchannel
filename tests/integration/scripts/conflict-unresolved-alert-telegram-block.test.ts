...
// conflict-unresolved probe. Plan §3.3 + §2.6.1 (REPEATABLE READ
// snapshot block stays IN PLACE before channel dispatch).
...
describe('conflict-unresolved probe — Telegram block gather-then-dispatch', () => {
  it('no offenders → one no_offenders row (snapshot block ran first)', async () => {
    ...
    // Default recipient_kind = email; no telegram dispatch on the
    // no_offenders short-circuit.
