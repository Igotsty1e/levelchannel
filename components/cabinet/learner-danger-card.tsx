...
// 2026-06-25 Bug 3 fix — переписан learner DangerZone в collapsible
// pattern по аналогии с TeacherDangerCard (components/teacher/profile/
// danger-card.tsx). Owner asked: «спрятать "Опасные действия" в отдельную
// кнопку и переименовать — сделать также как сделано в кабинете учителя».
//
// Visual contract:
//   - Closed by default. Title: «Удалить аккаунт или отозвать согласие».
//   - Header — кnop-trigger с chevron.
//   - Once expanded — Banner с danger tone, list of consequences,
//     Button primitive (danger + secondary variants).
//
// Backend contract без изменений:
//   - POST /api/account/consents/withdraw  (152-ФЗ ст.9 §5)
//   - POST /api/account/delete             (30-day soft-delete grace)
// Both clear session cookie + hard-redirect to /login on 2xx.

export function LearnerDangerCard() {
  const [open, setOpen] = useState(false)

  return (
    <section
      aria-labelledby="learner-profile-danger-title"
      style={cardStyle}
    >
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        aria-expanded={open}
        aria-controls="learner-profile-danger-body"
        style={triggerStyle}
      >
        <span style={triggerLabelStyle}>
          <span aria-hidden="true" style={dangerDotStyle} />
          <span
            id="learner-profile-danger-title"
      ...
      {open ? (
        <div id="learner-profile-danger-body" style={bodyStyle}>
          <Banner tone="danger" icon="⚠">
            <strong>Это разлогинит и заблокирует доступ.</strong> Расписание,
            оплаты и пакеты увидите только если оператор восстановит аккаунт.
          </Banner>
